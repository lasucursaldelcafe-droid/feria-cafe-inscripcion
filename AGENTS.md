...
El update script ya instala dependencias al arrancar la VM: `npm install` en `functions/` y los paquetes de `tools/requirements.txt` (vía `pip install --break-system-packages --user`, en `~/.local`). No necesitas reinstalarlas. Además, si existe el secreto/variable `SHEETS_WEB_APP_URL`, regenera `js/sheets-config.js` con esa URL para conectar los formularios al backend real automáticamente.
...
- **Backend de formularios (Apps Script + Google Sheets).** `js/form-submit.js` envía a la URL `/exec` de Apps Script definida en `js/sheets-config.js` (gitignored). Para conectarlo en la VM, añade el secreto `SHEETS_WEB_APP_URL` (URL `/exec`) y el update script generará `js/sheets-config.js`; o hazlo manualmente con `python3 tools/conectar_sheets.py --configurar-url "<URL>"`. Verifica con `python3 tools/conectar_sheets.py --verificar` (health `GET` debe devolver JSON `{ ok, forms: [...] }`). La Web App debe estar desplegada como **Ejecutar como: yo** + **Acceso: cualquier usuario**; si redirige a login devuelve HTML (no JSON) y los formularios fallan.

- **Modo local sin backend.** Sin URL configurada, los formularios entran en "Modo local": `js/form-submit.js` guarda el envío en `localStorage` y muestra confirmación de éxito. Es suficiente para probar el flujo de inscripción sin Apps Script.
